...
void Leaf::setup(float _branchAngle, int _season){
    ...
    rotation = _branchAngle + PI/8 + ofRandom(-(PI/4), PI/4); //bring in angle of branch
    
    rotation = rotation * (360/PI); //convert to degrees (from radians) to use for ofRotateZ() below, which expects degrees...
